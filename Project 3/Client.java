...
public class Client {
    ...
    // For the individual connections, again you can re-use the Connection class, and add some event handlers to process event codes that will be used to distinguish between peer-to-peer or cleint-server communications, or create a separate class called peerConnection. It is completely your choice.
...
{
    Client client;
    ObjectOutputStream peerToPeerOut;
    ObjectInputStream peerToPeerIn;
    ...
    void PeerToPeerHandler(InetAddress remotePeerIP, int remotePortNum, int remotePeerID, int findex, String fileHash)
    {
        try {
            // connect to peer
            Socket peerToPeer = new Socket(remotePeerIP, remotePortNum);
            this.peerToPeerOut = new ObjectOutputStream(peerToPeer.getOutputStream());
            this.peerToPeerIn = new ObjectInputStream(peerToPeer.getInputStream());
            ...
            System.out.println("The packet has been sent");
            System.out.println("You have connected to peer: " + client.s);

            boolean recievedCorrect = true;

            while(!recievedCorrect) {// while file not received correctly
                // request_file
                Packet p2 = new Packet();
                p2.event_type = 4;
                p2.sender = client.peerID;
                p2.peer_listen_port = client.peer_listen_port;
                p2.req_file_index = findex;
                send_packet_to_peer(p2);

                // receive_file_from_peer
                Packet p3 = new Packet();
                p3 = (Packet) this.peerToPeerIn.readObject();

                // verify file_hash
                String recievedHash = Connection.find_file_hash(p.DATA_BLOCK);
                System.out.println("Received File Hash: " + recievedHash);
                System.out.println("Expected File Hash: " + fileHash);

                // if correct, send positve ack, break
                if(recievedHash.equals(fileHash)){
                    Packet ack1 = new Packet();
                    ack1.gotFile = true;
                    ack1.event_type = 3;
                    ack1.sender = client.peerID;
                    ack1.req_file_index = findex;
                    send_packet_to_peer(ack1);
                    break;
                }
                // if incorrect, send negative ack, loop back
                else {
                    Packet ack2 = new Packet();
                    ack2.gotFile = false;
                    ack2.sender = client.peerID;
                    ack2.req_file_index = findex;
                    send_packet_to_peer(ack2);
                }
            }

            this.peerToPeerIn.close();
            this.peerToPeerOut.close();
            peerToPeer.close();
    ...
    void send_packet_to_peer(Packet p)
    {
        try
        {
            this.peerToPeerOut.writeObject(p);
        }
        catch(Exception e){
            System.out.println ("Could not send packet");
        }
